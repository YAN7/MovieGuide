...
(function(angular) {
    'use strict';
    // 1.0创建正在热映模块
    var app = angular.module("coming_soon", ["ngRoute", "http-server"]);
    ...
    app.controller("coming_soonController", [
        ...
        function($scope, $http, $routeParams, $route, Myservice) {
            ...
            // $http.get('./coming_soon/coming_soon.json').then(function(response) {
            // 将请求到的数据通过$scope.data暴露出去
            // $scope.data = response.data;

            //angular中的跨域请求需要加上JSON_CALLBACK参数
            // 由于angular不支持这种有点的参数，所以不能用angular的jsonp方法，需要自己封装一个跨域请求方法
            // $http.jsonp('http://api.douban.com//v2/movie/coming_soon?JSON_CALLBACK').then(function(data) {
            //   console.log(data);
            // })
